                ...
                <div class="overflow-auto bg-white">
                  ...
                  if(isset($_POST['submit'])){
                    if(isset($_POST['check_list'])){
                      ...
                      for($i=0;$i<count($check_list);$i++){
                        $delete = _delete("team","id=$check_list[$i]");
                      }
                  ...
                  <thead class="bg-white">
                    <tr>
                      ...
                      <th scope="col" class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5">Qualification</th>
                      <th scope="col" class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5">Email</th>
                      <th scope="col" class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5">Join Date</th>
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    if(isset($_GET['src'])){
                      $src = trim($_GET['src']);
                      $team = _get("team","id !='' AND (name='$src' OR qualification='$src')");
                    }elseif(isset($_GET['status'])){
                      $status = $_GET['status'];
                      $team =_get("team"," status='$status' ORDER BY id ASC");
                    }else{                    
                    $pagination = "ON";
                    ...
                    $team =_get("team","id !='' ORDER BY id DESC LIMIT $offset, $total_records_per_page");
                    $total_records = mysqli_num_rows(_getAll("team")); 

                    $total_no_of_pages = ceil($total_records / $total_records_per_page);
                    $second_last = $total_no_of_pages - 1;
                    }
                    while($data = mysqli_fetch_assoc($team)){
                    ?>
                      <tr class="hover:bg-gray-100">
                        ...
                        <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap lg:p-5"><?php echo $data['qualification']?></td>
                        <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap lg:p-5"><?php echo $data['mail']?></td>
